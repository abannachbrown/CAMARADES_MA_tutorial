...
<br>
Interleukin-1 receptor antagonist (IL-1RA) is an anti-inflammatory protein used clinically to treat rheumatoid arthritis and is considered a promising candidate therapy for stroke. In this systematic review and meta-analysis, we seek to assess the efficacy of IL-1RA in animal models of ischaemic stroke, the range of conditions under which efficacy has been tested, and whether the data appear to be confounded due to reported study quality and publication bias. The outcomes we will assess are post-stroke infarct volume (brain damage), mortality, and neurobehavioural outcomes e.g. motor deficits.
...
The tutorial comprises five main sections, each with multiple-choice questions to test your understanding:
<br>
**1.  Meta-analysis of continuous data** (infarct volume)

In the first section of the tutorial we will cover the steps of a meta-analysis of continuous data: 
<br>
...
In this section we will provide you with the data set describing neurobehavioural outcomes (continuous data) and prompts to insert code to analyse the data. 

You can copy and edit the code provided in Part 1 to perform the analyses. If you are unsure about what code to use, you can use the **Hint** button. You can compare your results to those in the paper, <a href="https://doi.org/10.1007/s12975-016-0489-z" target="_blank" rel="noopener noreferrer">McCann *et al*. 2016</a>. 
...
Now let's calculate an NMD effect size for each experiment (ESi) and add the effect size data as a new column in the infarctVol data set. The function "cbind" takes a table of data and binds a new column to the right side. Remember: each variable in this equation is a list of numbers, with each entry corresponding to a single experiment. In this format, the effect size tells us the percentage reduction in infarct volume in IL-1RA-treated animals compared to control animals.
<br> 
...
infarctVol2 <- subset(
  ...
  !(Drug.delivery.mode %in%  # specify the relevant variable
      c("Transgenic", "Vector", "IL-1RA Tg BM cells"))  # specify the variable levels to exclude from analysis  
)  
...
Next we combine the individual effect sizes with random effects meta-analysis using the function "metagen". This gives us an overall summary estimate of the effect of IL-1RA on infarct volume, compared to control.
...
quiz(caption = "Answer the following questions about the results of the meta-analysis:",
  ...
  question("How is the random effects model different to the fixed effect model and why? (check the model estimates and weights (%W))",
    ...
    answer("The fixed effect model estimates a mean of a distribution of effects while the random effects model estimates a common underying effect size"),
  ...
  question("How much heterogeneity is in the data set?",
    answer("tau^2 = 187.5 and I^2 = 82.3%, there is a significant amount of heterogeneity (p-value < 0.0001)", correct = TRUE),
  ...
  question("How do we interpret the value of I^2?",
    answer("82% of the variation across studies is due to heterogeneity rather than sampling error (chance)", correct = TRUE),
    answer("82% is close to 100% so there is considerable heterogeneity in the data set"),
...
infarctVolFor <- forest(
  ...
  # specify what information to print on the left side of the plot: the study labels
  leftcols = "Study.ID",
...
quiz(caption = "Questions:",
  ...
  question("What is printed on the plot?",
    answer("Effect sizes, the direction of the effect, and I2", correct = TRUE),
    answer("Effect sizes, standard errors, and tau2"),
    answer("Effect sizes, tau2, and the number of animals in each group"),
...
You can create a bar chart of these results using the code below. Don't worry about trying to understand the code at this stage, the flexibility offered by R means that it is very long!
...
quiz(caption = "Questions:",
  ...
  question("What does R^2 tell us?",
    ...
    answer("The proportion of heterogenity accounted for by reporting blinded induction of ischaemia"),
...
infarctVol2 <- subset(
  ...
  !(Drug.delivery.mode %in%  
      ...
      c("Transgenic", "Vector", "IL-1RA Tg BM cells"))  
) 
...
Have a look at the new R window that opened when you ran the code and check that the only studies that are remaining are those where the drug delivery mode is "Protein".
...
quiz(caption = "Questions:",
  ...
  question("Which routes might be the most clinically relevant?",
    answer("Administration into the brain (ICerebVentricular, Stereotactic) beacuse these routes are more direct", correct = TRUE),
    answer("Peripheral administration (SubCut, IVenous) because these routes are more often used in human patients", correct = TRUE),
    answer("ICerebVentricular (directly into the) because it has the highest efficacy"),
...
Have a look at the new R window that opened when you ran the code and check that the only studies that are remaining are those where the location of drug administration is "Central".
...
Take a look at the data sheet that has opened in a new R window and compare the outcome data to that from the infarct volume (continuous) data above.
...
quiz(caption = "Questions:",
  question("How is the forest plot different from the plot using the infarct volume data set?",
...
## 3.  Test you skills: meta-analysis 

Now you can explore the data set for neurobehavioural outcomes on your own.
We have only included experiments where IL-1RA was administered in protein form and motor / sensory outcomes were assessed.
...
# load the data set "IL1-RA_Neurobehaviour_Part1.4.csv" using the function read.csv().
...
# firstly load the data set "IL1-RA_Neurobehaviour_Part1.4.csv" using the function read.csv().
neurobeh <- read.csv("IL1-RA_Neurobehaviour_Part1.4.csv", fileEncoding = "UTF-8-BOM")
