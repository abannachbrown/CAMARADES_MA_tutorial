...
You can copy and edit the code provided in Part 1 to perform the analyses. If you are unsure about what code to use, use the **Hint(s)** button. You can compare your results to those in the paper, <a href="https://doi.org/10.1007/s12975-016-0489-z" target="_blank" rel="noopener noreferrer">McCann *et al*. 2016</a>. 

**4.  Small study effects** (infarct volume)

In this section we will assess the infarct volume data set for the presence of small study effects using funnel plot-based methods.
...
Now you can explore the data set for neurobehavioural outcomes by writing your own code. We have only included experiments where IL-1RA was administered in protein form and motor / sensory outcomes were assessed.
...
You can edit the code we used to analyse the infarct volume data above and compare your results to those in the paper, <a href="https://doi.org/10.1007/s12975-016-0489-z" target="_blank" rel="noopener noreferrer">McCann *et al*. 2016</a>.
...
Next select some variables to investigate as possible sources of heterogeneity using the function "metareg" e.g. species, sex, dose, time of administration, route of drug delivery, use of comorbid animals, method of induction of ischaemia, type of ischaemia, anaesthetic used, time of assessment, single/ multiple administrations.

Remember that the variable name you use in your meta-regression model must match the column name in the data sheet. To see a list of all the column names, you can use the function "colnames".
...
quiz(caption = "Questions:",
  ...
  question("What happens to the heterogeneity estimates when missing studies are added?",
    ...
    answer("The estimates of heterogeneity have decreased and are no longer"),
...
Next you will use neurobehavioural data to create a funnel plot, test for asymmetry, and impute missing studies using the "funnel, "metabias", and "trimfill" functions above. In part **3. Test your skills: meta-analysis** we used only those comparisons where IL-1RA was administered in protein form and motor / sensory outcomes were assessed. To assess for small study effects, we will use the entire neurobehavioural data set.

We have provided the first part of the code, to perform the meta-analysis, below:
...
```{r ex2_5_1, fig.width = 8.5, fig.height = 8, out.width="100%", out.height="100%", scaled = TRUE, exercise = TRUE, exercise.setup = "ex1_5_setup", exercise.lines=30}
...
quiz(caption = "Question:",
  question("What do the results of the analyses for small study effects tell us about the neurobehavioural data set?",
    answer("There is statistically significant evidence for funnel plot asymmetry and missing studies in the data set but the overall efficacy of IL-1RA is not shifted greatly when these missing studies are added", correct = TRUE),
    answer("There is no statistically significant evidence of funnel plot asymmetry but there are missing studies detected via trim and fill, evidence of publication bias"),
    answer("There is evidence of statistically significant funnel plot asymmetry and the addition of missing studies leads to a large reduction in the estimated efficacy of IL-1RA"),
    answer("There is strong evidence for publication bias and a significant number of studies missing from the data set"),
    random_answer_order = TRUE,
    allow_retry = TRUE
  )
)
...
This resource is supported by Charité 3R. For more information about the 3Rs at Charité – Universitätsmedizin Berlin, visit the [Charité 3R Toolbox](https://charite3r.charite.de/en/charite_3r_toolbox/){target="_blank"}. 
<center>
